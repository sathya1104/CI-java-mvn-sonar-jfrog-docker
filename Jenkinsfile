...
// artifactNumToKeepStr - Max # of builds to keep with artifacts	  
  ...
  stage('Build Docker Image'){
    steps{
      sh 'docker build -t sathya1104/springtest:$BUILD_NUMBER .'
    }
  }	  	 
  stage('Docker Container'){
    steps{
	  ...
	  sh 'docker login -u ${docker_user} -p ${docker_pass}'
      	  sh 'docker push sathya1104/springtest:$BUILD_NUMBER'
	  sh 'docker run -d -p 8050:8050 --name SpringbootApp sathya1104/springtest:$BUILD_NUMBER'
	  }
    }
  }
}	  	  
